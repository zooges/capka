...
struct SidebarView: View {
  ...
  /// Home-indicator inset — applied once on the account row so it sits low
  /// without the double-lift `safeAreaPadding` causes inside an already-safe layout.
  ...
  var onShare: (ChatSummary) -> Void
  var onExport: (ChatSummary) -> Void
    ...
    ZStack(alignment: .bottom) {
      ...
      if showAccountMenu {
        ...
        accountMenuCard
          ...
          .padding(.bottom, max(10, safeAreaBottom))
  ...
  private var content: some View {
    ...
    } else if let err = list.error, list.chats.isEmpty, search.isEmpty {
      VStack(spacing: 12) {
        Text(err)
          .font(.footnote)
          .foregroundStyle(Brand.muted)
  ...
  private func chatRow(_ row: ChatSummary) -> some View {
    // Title opens the chat; the trailing ⋮ is a separate control so it isn't
    // swallowed by the row button (same split as the web sidebar's MoreVertical).
    HStack(spacing: 0) {
      Button {
        onOpenChat(row)
      } label: {
      ...
      chatOptionsMenu(for: row)
        .padding(.trailing, 10)
    }
    .background(row.id == currentChatId ? Brand.accent : Color.clear)
    .clipShape(RoundedRectangle(cornerRadius: Brand.Radius.md, style: .continuous))
    .padding(.horizontal, 8)
    .contextMenu { chatOptionButtons(for: row) }
  }

  private func chatOptionsMenu(for row: ChatSummary) -> some View {
    Menu {
      chatOptionButtons(for: row)
    } label: {
  ...
  @ViewBuilder
  private func chatOptionButtons(for row: ChatSummary) -> some View {
    ...
    Button { onExport(row) } label: { Label("导出", systemImage: "square.and.arrow.down") }
    Button { onShare(row) } label: { Label("分享", systemImage: "square.and.arrow.up") }
    Button {
  ...
  private var accountFooter: some View {
    Button {
      ...
      HStack(spacing: 10) {
        UserAvatarView(user: session.user, size: 28)
    ...
    .buttonStyle(CapkaPressStyle())
    // Clear the home indicator with a little air — not flush, not double-padded.
    .padding(.top, 4)
    .padding(.bottom, max(10, safeAreaBottom > 0 ? safeAreaBottom / 2 + 4 : 10))
  }
...
struct SidebarChatRow: View {
  ...
  var body: some View {
    ...
    // Selected chrome lives on the outer HStack (title + ⋮) so the highlight
    // spans the whole row; keep only inner padding here.
    .padding(.leading, 12)
    .padding(.vertical, 8)
    .frame(maxWidth: .infinity, alignment: .leading)
    .contentShape(Rectangle())
...
/// Circular profile photo — Feishu SSO `user.image` when present, otherwise a
/// monogram like the web sidebar fallback.
struct UserAvatarView: View {
  ...
  private var imageURL: URL? {
    guard let raw = user?.image?.trimmingCharacters(in: .whitespacesAndNewlines),
  ...
  var body: some View {
    ...
    .clipShape(Circle())
    .overlay(Circle().stroke(Brand.line.opacity(0.6), lineWidth: 0.5))
