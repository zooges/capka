...
struct CapkaUser: Equatable, Codable {
  var id: String
  var name: String
  var email: String?
  var role: String?
  ...
  var status: String?
  /// Profile photo URL (Feishu SSO fills this from `avatar_url` / OIDC `picture`).
  var image: String?
}

struct ChatSummary: Identifiable, Equatable, Codable {
  var id: String
  var title: String?
  var pinned: Bool?
  var archived: Bool?
  ...
  /// `private` | `link` | `users` — who can open the public share URL.
  ...
  static func parseList(from data: Data) throws -> [ChatSummary] {
    ...
    return root.compactMap { item -> ChatSummary? in
      ...
      return ChatSummary(
        ...
        shareToken: obj["shareToken"] as? String
      )
    }
  }
  ...
  static func mutated(
    ...
    ChatSummary(
      ...
      shareToken: shareToken ?? base.shareToken
    )
  }
}
...
struct WorkspaceEntry: Identifiable, Equatable, Codable {
  var id: String { path }
  var name: String
  var path: String
  var isDirectory: Bool
  var size: Int64?
  /// ISO timestamp from the controller — used to invalidate the iOS preview cache.
...
extension ChatUIMessage {
  mutating func appendStreamedText(_ delta: String) {
    // Thinking is over once answer prose starts — leave the rail settled so it
    // stops pulsing/spinning while the rest of the turn streams.
    finishTrailingReasoning()
  ...
  mutating func upsertStreamedStep(_ step: MessageStep, mutate: ((inout MessageStep) -> Void)? = nil) {
    ...
    // A tool call after reasoning means that thinking block is finished.
    finishTrailingReasoning()
    var run = trailingActivity()
    var fresh = step
    mutate?(&fresh)
  ...
  /// Mark in-flight reasoning rows done so the rail stops pulsing once the model
  /// has moved on to tools or answer text.
  private mutating func finishTrailingReasoning() {
    guard case .activity(var run) = groups.last else { return }
    var changed = false
    for i in run.indices where run[i].kind == .reasoning && run[i].state == .running {
      run[i].state = .done
      changed = true
    }
  ...
  /// How much of the live reply the user can already see. Used when a mid-turn
  /// server snapshot lags behind SSE and must not clobber richer local groups.
  var liveRichness: Int {
    text.count * 10 + groups.count * 50 + steps.count
  }

  /// Prefer the side that already has more streamed prose/activity. Always keep
  /// `groups` as the source of truth — the UI renders those, not flat `text`.
  static func mergePreferringRicher(server: ChatUIMessage, local: ChatUIMessage?) -> ChatUIMessage {
    guard let local else { return server }
    var merged = server
    if local.liveRichness > server.liveRichness
      || (local.liveRichness == server.liveRichness && local.text.count > server.text.count)
      || (server.groups.isEmpty && !local.groups.isEmpty)
    {
      merged.groups = local.groups
      merged.rebuildFlattened()
    }
    // Cards that only exist live (ask / approval mid-turn) stay until the
    // finished snapshot includes them.
    var appendedCard = false
    for group in local.groups {
      switch group {
      case .ask(let card):
        if !merged.groups.contains(where: {
          if case .ask(let c) = $0 { return c.toolCallId == card.toolCallId }
          return false
        }) {
      ...
      case .approval(let card):
        if !merged.groups.contains(where: {
          if case .approval(let c) = $0 { return c.toolCallId == card.toolCallId }
          return false
        }) {
          merged.groups.append(group)
          appendedCard = true
    ...
    if appendedCard { merged.rebuildFlattened() }
    if local.isStreaming { merged.isStreaming = true }
    return merged
  }
}
