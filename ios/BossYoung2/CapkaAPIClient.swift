...
final class CapkaAPIClient: @unchecked Sendable {
  ...
  func getSession() async throws -> CapkaUser? {
    ...
    let name = (userObj["name"] as? String)
      ...
      ?? "用户"
    let image = (userObj["image"] as? String)?
  ...
  func patchChat(
  ...
  ) async throws -> (visibility: String?, shareToken: String?) {
    ...
    let json = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
    return (json?["visibility"] as? String, json?["shareToken"] as? String)
  }

  /// Downloads the chat as Markdown into a temp file for the system share sheet.
  func exportChatMarkdown(chatId: String, title: String?) async throws -> URL {
    var c = URLComponents(
      url: baseURL.appendingPathComponent("api/chats/\(chatId)/export"),
      resolvingAgainstBaseURL: false
    )!
    c.queryItems = [URLQueryItem(name: "format", value: "markdown")]
    var req = URLRequest(url: c.url!)
    ...
    let raw = (title?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap { $0.isEmpty ? nil : $0 } ?? "chat"
    let safe = raw
      .replacingOccurrences(of: "/", with: "-")
      .replacingOccurrences(of: ":", with: "-")
    let url = FileManager.default.temporaryDirectory
      .appendingPathComponent("\(safe)-\(chatId.prefix(8)).md")
  ...
  func fetchMessages(chatId: String) async throws -> [ChatUIMessage] {
    ...
    let messages = try Self.parseUIMessages(from: data)
    ChatDiskCache.shared.saveMessagesJSON(chatId: chatId, data: data)
    return messages
  }

  static func parseUIMessages(from data: Data) throws -> [ChatUIMessage] {
    guard let arr = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] else {
      throw CapkaAPIError.decoding
    }
    return arr.map { mapUIMessage($0) }
  }
  ...
  func listModels() async throws -> [ModelInfo] {
    ...
    req.setValue("application/json", forHTTPHeaderField: "Accept")
    // Admin can change providers any time — never serve a stale URLCache hit.
  ...
  func uploadFile(chatId: String? = nil, projectId: String? = nil, fileURL: URL, destDir: String = ".") async throws -> (name: String, type: String) {
    ...
    // Keep the bytes that just left the phone — opening the upload must not
    // round-trip back through the office host.
    let cachePath = destDir == "." ? filename : "\(destDir)/\(filename)"
    PreviewDiskCache.ingestLocalFile(
      fileURL,
      chatId: chatId,
      projectId: projectId,
      path: cachePath,
  ...
  /// One-shot folder import via `/api/folders/upload` (chunks of relative paths).
  /// Writes under `/workspace/<folderName>/…`.
  func uploadFolder(
    chatId: String? = nil,
    projectId: String? = nil,
    folderURL: URL
  ) async throws -> (name: String, count: Int) {
    let folderName = Self.sanitizeFolderName(folderURL.lastPathComponent)
    let files = try Self.collectFolderFiles(root: folderURL)
    guard !files.isEmpty else { return (folderName, 0) }

    let chunkSize = 40
    var uploaded = 0
    for start in stride(from: 0, to: files.count, by: chunkSize) {
      let slice = Array(files[start..<min(start + chunkSize, files.count)])
      let boundary = "Boundary-\(UUID().uuidString)"
      var req = URLRequest(url: baseURL.appendingPathComponent("api/folders/upload"))
      req.httpMethod = "POST"
      req.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
      req.setValue("application/json", forHTTPHeaderField: "Accept")
      ...
      for item in slice {
        ...
        let mime = mimeType(for: item.url)
        append("--\(boundary)\r\n")
        // Server uses the File's name as the path relative to the folder root.
        append("Content-Disposition: form-data; name=\"files\"; filename=\"\(item.relativePath)\"\r\n")
        append("Content-Type: \(mime)\r\n\r\n")
        body.append(data)
        append("\r\n")
      }
  ...
  private static let skipDirNames: Set<String> = [
    "node_modules", ".git", "__pycache__", ".DS_Store", "Pods", ".venv", "venv",
  ]

  private static func collectFolderFiles(root: URL) throws -> [FolderFile] {
    ...
    while let url = enumerator.nextObject() as? URL {
      ...
      if skipDirNames.contains(url.lastPathComponent) { continue }
      // Skip oversized files (>100 MB) — mirrors server FOLDER_MAX_FILE_MB.
      if let size = values.fileSize, size > 100 * 1024 * 1024 { continue }
      let full = url.standardizedFileURL.path
      guard full.hasPrefix(rootPath + "/") else { continue }
      let rel = String(full.dropFirst(rootPath.count + 1))
      if rel.isEmpty { continue }
      out.append(FolderFile(url: url, relativePath: rel))
  ...
  func listFiles(chatId: String? = nil, projectId: String? = nil, path: String = ".") async throws -> [WorkspaceEntry] {
    ...
    return entries.compactMap { e in
      ...
      return WorkspaceEntry(
        ...
        isDirectory: isDir,
        size: size,
        modifiedAt: e["modifiedAt"] as? String
      )
    }
  }
  ...
  /// Quick Look needs a real file on disk. Always lands in the Application
  /// Support preview cache so a second tap (chat chip or workspace row) is local.
  func downloadToTemp(
    chatId: String? = nil,
    projectId: String? = nil,
    path: String,
    ...
    var req = URLRequest(url: url)
    // URLCache keys by URL alone and knows nothing about workspace mtimes —
    // once PreviewDiskCache decides to re-download, the bytes must be fresh.
    req.cachePolicy = .reloadIgnoringLocalCacheData
    do {
      ...
      return try PreviewDiskCache.store(
        downloaded: tempURL,
        chatId: chatId,
        projectId: projectId,
        path: path,
      ...
      throw CancellationError()
    } catch let err as CapkaAPIError {
      throw err
    } catch {
  ...
  /// Background-warm workspace files into the on-disk preview cache.
  func warmWorkspaceFiles(
    chatId: String? = nil,
    projectId: String? = nil,
    entries: [WorkspaceEntry],
    maxCount: Int = 16,
    maxBytes: Int64 = 12 * 1024 * 1024
  ) {
    let candidates = entries.filter { entry in
      guard !entry.isDirectory else { return false }
      // No size reported means no upper bound — not worth risking on cellular.
      guard let size = entry.size, size <= maxBytes else { return false }
      return PreviewDiskCache.lookup(
        chatId: chatId,
        projectId: projectId,
        path: entry.path,
        filename: entry.name,
    ...
    Task.detached(priority: .utility) {
      for entry in candidates.prefix(maxCount) {
        _ = try? await self.downloadToTemp(
          chatId: chatId,
          projectId: projectId,
          path: entry.path,
          filename: entry.name,
  ...
  private func send(_ req: URLRequest) async throws -> (Data, URLResponse) {
    do {
      ...
      // Debounced sidebar search cancels the previous keystroke's request —
      // that is not a network failure and must not surface as one.
      throw CancellationError()
    } catch {
      throw CapkaAPIError.message(Self.friendlyNetworkError(error))
    }
      ...
      for (index, part) in parts.enumerated() {
        ...
        if type == "reasoning" {
          ...
          let reasoningStep = MessageStep(
            id: "\(id)-reason-\(index)",
            kind: .reasoning,
            // Snapshot parts are complete blocks. Live SSE re-marks the active
            // row `.running`; treating every reasoning part as running while
            // `taskStatus == running` left finished thinking pulsing forever.
            state: .done,
            label: "推理",
            icon: "lightbulb",
            detail: t
        ...
        if type.contains("tool") {
          ...
          if name == "ask" {
            // Prefer the dedicated askForm; fall back to the tool input so a
            // mid-turn snapshot still builds the question card.
            let form = (p["askForm"] as? [String: Any])
              ?? (p["input"] as? [String: Any])
              ?? [:]
            var part = p
            if part["state"] == nil { part["state"] = "input-available" }
            flushActivity()
            groups.append(.ask(parseAskCard(part: part, form: form)))
            continue
          }
...
/// On-disk preview cache. Workspace files live on the office host — without this
/// every Quick Look tap re-downloads the whole PDF/docx over the phone's link.
/// Lives under Application Support (not tmp) so iOS does not purge it between taps.
enum PreviewDiskCache {
  private static var root: URL {
    let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
      .appendingPathComponent("capka-preview-cache", isDirectory: true)
  ...
  static func key(
    ...
    // Prefer mtime, then size; chat chips often lack both — still cache under a
    // stable stamp so the second open of the same path is local.
    let stamp = modifiedAt ?? (size.map { String($0) } ?? "open")
    let raw = "\(scope)|\(path)|\(stamp)"
    let digest = SHA256.hash(data: Data(raw.utf8))
    return digest.map { String(format: "%02x", $0) }.joined()
  }

  /// Try stamped key first, then the generic "open" key used by chat chips.
  static func lookup(
    chatId: String?,
    projectId: String?,
    path: String,
    ...
    // With a server mtime in hand, an unstamped hit may be an older version of
    // the same path — never fall back past what the metadata can vouch for.
    var tried = Set<String>()
    let variants: [(Int64?, String?)] = modifiedAt != nil
      ? [(size, modifiedAt)]
      : [(size, nil), (nil, nil)]
    for (s, m) in variants {
      let k = key(chatId: chatId, projectId: projectId, path: path, size: s, modifiedAt: m)
      guard tried.insert(k).inserted else { continue }
      if let hit = cachedURL(key: k, filename: filename) { return hit }
    }
    return nil
  }

  static func cachedURL(key: String, filename: String) -> URL? {
    let dest = cachedFileURL(key: key, filename: filename)
    guard FileManager.default.fileExists(atPath: dest.path) else { return nil }
    // Touch access time so a simple LRU sweep can drop cold entries later.
  ...
  static func store(
    ...
    let primary = key(chatId: chatId, projectId: projectId, path: path, size: size, modifiedAt: modifiedAt)
    let dest = try write(downloaded: downloaded, key: primary, filename: filename, move: true)
    // Mirror under the chat-chip "open" stamp so taps without mtime still hit.
    let openKey = key(chatId: chatId, projectId: projectId, path: path, size: nil, modifiedAt: nil)
    if openKey != primary {
      let openDest = cachedFileURL(key: openKey, filename: filename)
      try? FileManager.default.removeItem(at: openDest)
      try? FileManager.default.copyItem(at: dest, to: openDest)
    }
    pruneIfNeeded(maxBytes: 200 * 1024 * 1024)
    return dest
  }

  /// Copy a file the user just picked/captured into the preview cache (upload path).
  static func ingestLocalFile(
    _ source: URL,
    chatId: String?,
    projectId: String?,
    path: String,
    ...
    let primary = key(chatId: chatId, projectId: projectId, path: path, size: size, modifiedAt: nil)
    let openKey = key(chatId: chatId, projectId: projectId, path: path, size: nil, modifiedAt: nil)
    do {
      let dest = try write(downloaded: source, key: primary, filename: filename, move: false)
      if openKey != primary {
        let openDest = cachedFileURL(key: openKey, filename: filename)
        try? FileManager.default.removeItem(at: openDest)
        try FileManager.default.copyItem(at: dest, to: openDest)
      ...
      // Cache is best-effort — upload already succeeded.
    }
  }

  private static func write(downloaded: URL, key: String, filename: String, move: Bool) throws -> URL {
  ...
  private static func cachedFileURL(key: String, filename: String) -> URL {
    let safe = filename.replacingOccurrences(of: "/", with: "_")
    let ext = (safe as NSString).pathExtension
    let base = ext.isEmpty ? key : "\(key).\(ext)"
    return root.appendingPathComponent(base.isEmpty ? key : base)
  }

  private static func pruneIfNeeded(maxBytes: Int64) {
    ...
    var total: Int64 = 0
    var ranked: [(url: URL, size: Int64, date: Date)] = []
    for url in files {
      let values = try? url.resourceValues(forKeys: [.fileSizeKey, .contentModificationDateKey])
      let size = Int64(values?.fileSize ?? 0)
      total += size
      ranked.append((url, size, values?.contentModificationDate ?? .distantPast))
    }
    guard total > maxBytes else { return }
    for item in ranked.sorted(by: { $0.date < $1.date }) {
