...
struct ActivityRail: View {
  ...
  /// True only while a step in *this* rail is still running — not for the whole
  /// message turn. Answer text can stream long after thinking finished.
  private var isLive: Bool { steps.contains { $0.state == .running } }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      Button {
        ...
        HStack(spacing: 6) {
          Text(summary)
            .font(.system(size: 13))
            .foregroundStyle(Brand.muted)
            .modifier(PulseWhile(active: isLive))
      ...
      .onChange(of: isLive) { _, live in
        ...
        // Avoid re-animating an already-open rail on every step tick —
        // that was bouncing「思考」up and down while reasoning streamed.
        if live {
          if !expanded {
            withAnimation(Motion.easeOut(0.22)) { expanded = true }
          }
        } else if expanded {
          withAnimation(Motion.easeOut(0.22)) { expanded = false }
        }
      }

      if expanded {
        VStack(alignment: .leading, spacing: 0) {
          // Never entrance-animate steps: remounting a finished chat used to
          // replay opacity-in on every historical reasoning row.
...
struct AttachmentTile: View {
  let name: String
  let kind: FileKind
  /// When set, image tiles load a real thumbnail from the sandbox (like the web).
  ...
  private var tile: some View {
    VStack(spacing: 4) {
      ZStack {
        ...
        if kind.isImage, let chatId {
          SandboxImageThumb(chatId: chatId, path: workspacePath ?? name)
            .frame(width: 88, height: 88)
...
/// 88pt cover thumbnail for a workspace image. Prefers the on-disk preview cache
/// so scrolling a chat of screenshots does not re-hit the office host.
struct SandboxImageThumb: View {
  let chatId: String
  let path: String
        ...
        AsyncImage(url: CapkaAPIClient.shared.downloadURL(chatId: chatId, path: path, inline: true)) { phase in
          ...
          case .success(let image):
            image
              .resizable()
              .scaledToFill()
              .task { await cacheRemoteIfNeeded() }
          case .failure:
            Image(systemName: "photo")
              .font(.system(size: 24))
              .foregroundStyle(FileKind.image.color)
              .frame(maxWidth: .infinity, maxHeight: .infinity)
              .background(FileKind.image.tint)
          default:
            ProgressView()
              .controlSize(.small)
              .tint(Brand.muted)
  ...
  private func loadLocal() {
    ...
    guard let url = PreviewDiskCache.lookup(
      chatId: chatId,
      projectId: nil,
      path: path,
      filename: name,
    ...
    let ui = Platform.image(from: data)
    else { return }
    #if os(macOS)
      localImage = Image(nsImage: ui)
    #else
      localImage = Image(uiImage: ui)
    #endif
  }

  private func cacheRemoteIfNeeded() async {
    let name = (path as NSString).lastPathComponent
    _ = try? await CapkaAPIClient.shared.downloadToTemp(
      chatId: chatId,
      path: path,
      filename: name
    )
...
struct AskCardView: View {
  ...
  var body: some View {
    ...
    .frame(maxWidth: .infinity, alignment: .leading)
    .padding(.vertical, 4)
    // No entrance animation — a remount mid-turn made the question card flash
    // in late / look like it needed a refresh.
  }
...
/// where a steady dimmed state stands in for the throb. Stops cleanly when
/// `active` flips off — no leftover forever-animation after thinking ends.
