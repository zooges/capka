...
struct WorkspaceFilesView: View {
  ...
  init(chatId: String?, projectId: String?, title: String) {
    self.chatId = chatId
  ...
  private func row(_ entry: WorkspaceEntry) -> some View {
    ...
    if entry.isDirectory {
      ...
      // Quick Look downloads through the session; a bare ShareLink would have
      // handed out a URL the receiving app can't authenticate. Its own share
      // button covers exporting the file.
      Button {
        preview.open(chatId: chatId, projectId: projectId, path: entry.path, name: entry.name)
      } label: {
        content
      }
      .buttonStyle(CapkaPressStyle())
